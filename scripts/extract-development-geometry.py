...
def main():
    ...
    ap.add_argument("--match-index", action="append", default=[], help='"레코드인덱스=CONST" — ALIAS 빈(무명) 피처를 --search로 찾은 #인덱스로 지정')
    ...
    custom = bool(args.match or args.match_index)
    match = dict(m.split("=", 1) for m in args.match) if args.match else ({} if args.match_index else DEFAULT_MATCH)
    areas = dict((k, float(v)) for k, v in (m.split("=", 1) for m in args.area)) if args.area else (DEFAULT_AREA if not custom else {})
    out = args.out or (DEFAULT_OUT if not custom else None)
    if not out:
        sys.exit("--out 필요")
    ...
    # 무명(ALIAS 빈) 피처: --search로 확인한 레코드 인덱스로 직접 지정.
    for m in args.match_index:
        idx_s, const = m.split("=", 1)
        idx = int(idx_s)
        label = recs[idx][1].get("REMARK") or recs[idx][1].get("ALIAS") or f"#{idx}"
        want[idx] = (label, const)
