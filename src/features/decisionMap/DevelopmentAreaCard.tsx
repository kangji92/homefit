 ...
 * 달리 점수/순위가 없다 — fitScore 미반영. 클릭 시 Decision Map(사업 상세)로 이동.
 */
export function DevelopmentAreaCard({ area, href = "/strategy?view=map" }: DevelopmentAreaCardProps) {
  ...
  return (
    <Link href={href} className="border-border bg-surface hover:border-primary block rounded-xl border p-4">
      ...
      <p className="text-muted-foreground mt-2 text-[11px]">
        판단 보조 정보 · 적합도 점수에 반영되지 않아요. 지도에서 자세히 보기 →
      </p>
    </Link>
  );
}
