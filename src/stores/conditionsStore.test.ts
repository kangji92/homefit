...
describe("conditionsStore", () => {
  it("기본 상태로 초기화된다", () => {
    const s = get();
    expect(s.conditions.maxBudget).toBe(0);
  ...
  it("patchConditions는 나머지 필드를 보존하며 병합한다", () => {
    get().patchConditions({ maxBudget: 100000 });
    const s = get();
    expect(s.conditions.maxBudget).toBe(100000);
    // 병합이므로 다른 필드는 그대로
    expect(s.conditions.workplaces).toHaveLength(2);
  ...
  it("reset은 기본값으로 되돌린다", () => {
    get().patchConditions({ maxBudget: 123456 });
    get().completeOnboarding();
    get().setStep(4);
    get().reset();
    const s = get();
  ...
  it("변경사항을 localStorage에 영속하되 hasHydrated는 저장하지 않는다", () => {
    ...
    expect(persisted.state.conditions.maxBudget).toBe(100000);
